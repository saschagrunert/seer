...
gitSpec = parallel $ do
    ...
    it "should fail with a real 'git test' command"
        $              isLeft
        <$>            runGitCommandIO "test" "."
        `shouldReturn` True

    it "should succeed with a mocked 'git status' command (runGitCommandIO)"
        $ do
              let fixture = def
                      { _readProcessWithExitCode' = \_ _ _ ->
                          return (ExitSuccess, "stdOut", "")
    ...
    it
            "should fail with a mocked non existing 'git' executable (runGitCommandIO)"
        $ do
              let fixture = def
                      { _readProcessWithExitCode' = \_ _ _ ->
                          return (ExitSuccess, "", "")
